...
(function () {
    ...
    // ─────────────────────────────────────────────
    // GRADING SETTINGS STORAGE
    // ─────────────────────────────────────────────
    const SETTINGS_KEY = "AIgrader_GradeSettings_v1";

    function loadGradingSettings(assignmentId) {
        const all = GM_getValue(SETTINGS_KEY, {});
        return all[String(assignmentId)] || getDefaultGradingSettings();
    }

    function saveGradingSettings(assignmentId, settings) {
        const all = GM_getValue(SETTINGS_KEY, {});
        all[String(assignmentId)] = settings;
    ...
    function createLauncher() {
        document.getElementById("aigrader-launcher")?.remove();
        const b = el("button", {
            position: "fixed", bottom: "20px", right: "20px",
            width: "62px", height: "62px", borderRadius: "50%",
            background: "#2563eb", color: "#fff", border: "none",
            fontWeight: "bold", cursor: "pointer", zIndex: "999999",
            fontSize: "13px", boxShadow: "0 4px 18px rgba(37,99,235,0.45)",
            lineHeight: "1.2", transition: "transform 0.15s"
        }, { id: "aigrader-launcher", textContent: "AI\nGrade" });
        b.style.whiteSpace = "pre";
        b.onmouseenter = () => b.style.transform = "scale(1.08)";
        b.onmouseleave = () => b.style.transform = "scale(1)";
        b.onclick = injectPanel;
    ...
    function buildGradeView(db) {
        ...
        if (state.detectedCourseId && state.assignmentsLoading) {
            ...
            courseSection.appendChild(buildSelectWithIds(
                ...
                (id, assignment) => {
                    ...
                    // Load grading settings for this assignment
                    if (id) {
                        state.gradingSettings = loadGradingSettings(id);
        ...
        if (state.selectedAssignmentId) {
            ...
            settingsBtn.onclick = () => {
                state.gradingSettings = loadGradingSettings(state.selectedAssignmentId);
                state.view = "settings";
                render();
            };
        ...
        if (state.selectedAssignmentId) {
            const settings = loadGradingSettings(state.selectedAssignmentId);
            const summaryParts = [
    ...
    function buildSettingsEditorView() {
        ...
        const settings = state.gradingSettings || loadGradingSettings(assignmentId);
        ...
        saveBtn.onclick = () => {
            saveGradingSettings(assignmentId, settings);
    ...
    function handleGrade() {
        ...
        if (state.selectedAssignmentId) {
            // Using Canvas-detected assignment
            item = getItemByAssignmentId(db, state.selectedAssignmentId);
            settings = loadGradingSettings(state.selectedAssignmentId);
