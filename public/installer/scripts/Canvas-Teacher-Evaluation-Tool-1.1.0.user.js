...
(function () {
  ...
  function renderInputForm(courseInfo) {
    ...
    return '<div class="cte-form">' +
      ...
      '<div class="cte-form-group"><label>Course Hours</label><input type="number" id="cte-course-hours" placeholder="e.g. 100" min="1" max="1000" /><div class="cte-hint">Total instructional hours for the course</div></div>' +
      '<div class="cte-form-group"><label>Term Length (weeks)</label><input type="number" id="cte-term-weeks" placeholder="e.g. 16" min="1" max="52" /></div>' +
      '<div class="cte-form-group"><label>Delivery Type</label><select id="cte-delivery-type"><option value="online">Online</option><option value="hybrid">Hybrid</option><option value="in-person">In-Person</option></select><div class="cte-hint">Adjusts expectations for communication & engagement</div></div>' +
  ...
  function renderDashboard(scores, metrics, flags, recommendations, context) {
    ...
    return '<div class="cte-context-bar">' +
      ...
      '<span class="cte-context-chip">\u{1F4E1} ' + escHtml(context.deliveryType) + '</span>' +
    ...
    '<div class="cte-score-hero">' +
      ...
      '<div class="cte-score-hero-text"><h2>' + overallLabel.label + '</h2><p>Weighted evaluation across ' + enabledCount + ' categories. Normalized for a ' + context.courseHours + '-hour ' + context.deliveryType + ' course over ' + context.termWeeks + ' weeks.</p></div>' +
    '</div>' +
  ...
  async function launchEvaluation() {
    ...
    document.getElementById("cte-run-btn").addEventListener("click", async function () {
      ...
      var courseHours = parseInt(document.getElementById("cte-course-hours").value, 10) || 100;
      var termWeeks = parseInt(document.getElementById("cte-term-weeks").value, 10) || 16;
      var deliveryType = document.getElementById("cte-delivery-type").value || "online";
  ...
  function init() {
    ...
    if (getCourseId()) {
      ...
      group.className = "cte-btn-group";

      var evalBtn = document.createElement("button");
      evalBtn.id = "cte-trigger-btn";
      evalBtn.className = "cte-round-btn";
      evalBtn.innerHTML = '\u{1F4CA}<span class="cte-tooltip">Evaluate Course</span>';
      evalBtn.addEventListener("click", toggleEvaluation);

      var settingsBtn = document.createElement("button");
      settingsBtn.id = "cte-settings-btn";
      settingsBtn.className = "cte-round-btn";
      settingsBtn.innerHTML = '\u2699\uFE0F<span class="cte-tooltip">Settings</span>';
      settingsBtn.addEventListener("click", openSettings);

      group.appendChild(evalBtn);
